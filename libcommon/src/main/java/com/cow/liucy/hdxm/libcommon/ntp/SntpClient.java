package com.cow.liucy.hdxm.libcommon.ntp;
/*
...
public class SntpClient {
    ...
    private static final int NTP_VERSION = 4;
    ...
    public SntpClient(String host,int ntpPort){
        this.host=host;
        this.ntpPort=ntpPort;
    }

    public  long[] requestTime() throws IOException{
        return requestTime(1000.0f,100000.0f,750,30*1000);
    }
     ...
     *
     */
   private synchronized long[] requestTime(float rootDelayMax,
        ...
        try {
            ...
            DatagramPacket request = new DatagramPacket(buffer, buffer.length, address, getNtpPort());
            ...
            AppLogger.e( "---- SNTP successful response from " + host +">>rootDelay:"+rootDelay+">>rootDispersion:"+rootDispersion+">>delay:"+delay);
            ...
            AppLogger.e( "---- SNTP request failed for " + host);
            throw e;
        } finally {
            if (socket != null) {
                socket.close();
    ...
    long sntpTime(long[] response) {
        ...
        String d = format.format(responseTime+clockOffset);
//        AppLogger.e(">>>>ntp time:"+ d);
